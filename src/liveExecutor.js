...
async function jupiterOrder({ inputMint, outputMint, amount, priorityFeeMicroLamports = null }) {
  requireLiveExecution();
  ...
  if (Number.isFinite(Number(priorityFeeMicroLamports)) && Number(priorityFeeMicroLamports) > 0) {
    url.searchParams.set('priorityFeeMicroLamports', String(Math.floor(Number(priorityFeeMicroLamports))));
  }
...
export async function executeJupiterSwap({ inputMint, outputMint, amount, priorityFeeMicroLamports = null }) {
  const order = await jupiterOrder({ inputMint, outputMint, amount, priorityFeeMicroLamports });
